...
const translations = {
  english: {
    ...
    subtitle: 'An interactive, visual step-by-step guide to praying the Holy Rosary, Chaplet of St. Michael, and Rosary of the Seven Sorrows.',
    tabVisual: 'Visual Rosary Guide',
    tabPdfs: 'PDF Prayer Books',
    selectDevotional: 'Select Devotional',
    marianTitle: 'Holy Rosary (Dominican)',
    marianDesc: '5 Decades. Meditations on the life of Christ.',
    stmichaelTitle: 'Chaplet of St. Michael',
    stmichaelDesc: '9 Salutations in honor of the 9 Angel Choirs.',
    sevensorrowsTitle: 'Rosary of Seven Sorrows',
    sevensorrowsDesc: '7 Meditations on Our Lady of Sorrows.',
    mysteries: 'Mysteries',
    beadMap: 'Rosary Bead Map',
    legendCrucifix: 'Crucifix (Start / Creed)',
    legendOurFather: 'Large Bead (Our Father)',
    legendHailMary: 'Small Bead (Hail Mary)',
    legendActive: 'Active Bead Indicator',
    step: 'Step',
    of: 'of',
    meditation: 'Meditation & Mystery',
    previous: 'Previous',
    next: 'Next Step',
    startOver: 'Start Over',
    decadeGuide: 'Decade Guide',
    progress: 'Progress:',
    legendInstruction: '* Click on any bead on the map to jump directly to that prayer. Amber beads are "Our Father" prayers, grey/purple are "Hail Marys".',
    downloadPdf: 'Download PDF',
    size: 'Size:',
    noPdfs: 'No PDF booklets available yet.',
    loadingPdfs: 'Loading PDF library...'
  },
  swahili: {
    title: 'Jinsi ya Kusali Rozari',
    subtitle: 'Mwongozo shirikishi na wa picha hatua kwa hatua kusali Rozari Takatifu, Koreti ya Malaika Mkuu Mikaeli, na Rozari ya Masikitiko Saba ya Bikira Maria.',
    tabVisual: 'Mwongozo wa Rozari (Picha)',
    tabPdfs: 'Vitabu vya Sala (PDF)',
    selectDevotional: 'Chagua Ibada',
    marianTitle: 'Rozari Takatifu (Dominiko)',
    marianDesc: 'Mafungu 5. Tafakari za maisha ya Yesu na Bikira Maria.',
    stmichaelTitle: 'Koreti ya Mtakatifu Mikaeli',
    stmichaelDesc: 'Salamu 9 kwa heshima ya Kwaya 9 za Malaika.',
    sevensorrowsTitle: 'Rozari ya Masikitiko Saba',
    sevensorrowsDesc: 'Tafakari 7 za Masikitiko ya Bikira Maria.',
    mysteries: 'Mafumbo / Siri',
    beadMap: 'Ramani ya Rozari',
    legendCrucifix: 'Msalaba (Kuanza / Nasadiki)',
    legendOurFather: 'Punje Kubwa (Baba Yetu)',
    legendHailMary: 'Punje Ndogo (Salamu Maria)',
    legendActive: 'Punje Inayosaliwa Sasa',
    step: 'Hatua',
    of: 'kati ya',
    meditation: 'Tafakari na Fumbo',
    previous: 'Nyuma',
    next: 'Hatua Inayofuata',
    startOver: 'Anza Tena',
    decadeGuide: 'Maendeleo ya Rozari',
    progress: 'Kipimo:',
    legendInstruction: '* Bonyeza punje yoyote kwenye ramani kwenda moja kwa moja kwenye sala hiyo. Punje za njano ni "Baba Yetu", za zambarau ni "Salamu Maria".',
    downloadPdf: 'Pakua PDF',
    size: 'Ukubwa:',
    noPdfs: 'Bado hakuna vitabu vya PDF vilivyopakiwa.',
    loadingPdfs: 'Inapakia maktaba ya PDF...'
  }
};
...
export default function DevotionalPrayers() {
    ...
    async function fetchPdfs() {
      try {
        ...
        if (data && data.length > 0) {
          ...
          setPdfList(DEFAULT_PDFS);
        }
      } catch (err) {
        console.warn('Could not load PDFs from database, using local fallback:', err);
        setPdfList(DEFAULT_PDFS);
      } finally {
  ...
  const t = translations[rosaryLanguage];
  ...
  const buildMarianBeads = (): Bead[] => {
    ...
    list.push({
      id: 1, type: 'creed', label: rosaryLanguage === 'english' ? '1. Crucifix (Apostles’ Creed)' : '1. Msalaba (Nasadiki)', prayerName: prayersTemplates.creed.nameEn,
    ...
    list.push({
      id: 2, type: 'our-father', label: rosaryLanguage === 'english' ? '2. First Large Bead (Our Father)' : '2. Punje Kubwa ya Kwanza (Baba Yetu)', prayerName: prayersTemplates.ourFather.nameEn,
    ...
    for (let i = 0; i < 3; i++) {
      list.push({
        id: 3 + i, type: 'hail-mary', label: rosaryLanguage === 'english' ? `3. Small Bead ${i+1} (Hail Mary)` : `3. Punje Ndogo ${i+1} (Salamu Maria)`, prayerName: prayersTemplates.hailMary.nameEn,
    ...
    list.push({
      id: 6, type: 'glory-be', label: rosaryLanguage === 'english' ? '4. Chain Space (Glory Be)' : '4. Nafasi/Kamba (Atukuzwe Baba)', prayerName: prayersTemplates.gloryBe.nameEn,
    ...
    for (let decade = 0; decade < 5; decade++) {
      ...
      list.push({
        id: beadId++, type: 'our-father', label: rosaryLanguage === 'english' ? `5. Decade ${decade+1} - Our Father` : `5. Fungu la ${decade+1} - Baba Yetu`, prayerName: prayersTemplates.ourFather.nameEn,
        prayerTextEn: prayersTemplates.ourFather.en, prayerTextSw: prayersTemplates.ourFather.sw,
      ...
      for (let hm = 0; hm < 10; hm++) {
        ...
        list.push({
          id: beadId++, type: 'hail-mary', label: rosaryLanguage === 'english' ? `6. Decade ${decade+1} - Hail Mary ${hm+1}` : `6. Fungu la ${decade+1} - Salamu Maria ${hm+1}`, prayerName: prayersTemplates.hailMary.nameEn,
          prayerTextEn: prayersTemplates.hailMary.en, prayerTextSw: prayersTemplates.hailMary.sw,
      ...
      list.push({
        id: beadId++, type: 'glory-be', label: rosaryLanguage === 'english' ? `7. Decade ${decade+1} - Glory Be & Fatima` : `7. Fungu la ${decade+1} - Atukuzwe & Fatima`, prayerName: `${prayersTemplates.gloryBe.nameEn} & ${prayersTemplates.fatima.nameEn}`,
    ...
    list.push({
      id: beadId++, type: 'conclusion', label: rosaryLanguage === 'english' ? '8. Center Medal (Hail Holy Queen)' : '8. Medali ya Kati (Salamu Malkia)', prayerName: prayersTemplates.hailHolyQueen.nameEn,
  ...
  const buildStMichaelBeads = (): Bead[] => {
    ...
    // Medal (Top Center)
    list.push({
      id: 1, type: 'intro', label: rosaryLanguage === 'english' ? '1. Medal Opening Invocation' : '1. Medali ya Kuanzia', prayerName: 'Opening Invocation',
    ...
    for (let s = 0; s < 9; s++) {
      ...
      list.push({
        id: beadId++, type: 'salutation', label: rosaryLanguage === 'english' ? `2. Salutation ${s+1} Meditation` : `2. Salamu ya ${s+1} Tafakari`, prayerName: `Salutation ${s+1}`,
      ...
      // 1 Our Father
      list.push({
        id: beadId++, type: 'our-father', label: rosaryLanguage === 'english' ? `3. Salutation ${s+1} - Our Father` : `3. Salamu ya ${s+1} - Baba Yetu`, prayerName: prayersTemplates.ourFather.nameEn,
        prayerTextEn: prayersTemplates.ourFather.en, prayerTextSw: prayersTemplates.ourFather.sw,
      ...
      for (let h = 0; h < 3; h++) {
        ...
        list.push({
          id: beadId++, type: 'hail-mary', label: rosaryLanguage === 'english' ? `4. Salutation ${s+1} - Hail Mary ${h+1}` : `4. Salamu ya ${s+1} - Salamu Maria ${h+1}`, prayerName: prayersTemplates.hailMary.nameEn,
          prayerTextEn: prayersTemplates.hailMary.en, prayerTextSw: prayersTemplates.hailMary.sw,
    ...
    for (let a = 0; a < 4; a++) {
      list.push({
        id: beadId++, type: 'our-father', label: rosaryLanguage === 'english' ? `5. Our Father in honor of ${angels[a]}` : `5. Baba Yetu kwa heshima ya ${malaika[a]}`, prayerName: `Our Father - ${angels[a]}`,
        prayerTextEn: prayersTemplates.ourFather.en, prayerTextSw: prayersTemplates.ourFather.sw,
    ...
    list.push({
      id: beadId++, type: 'conclusion', label: rosaryLanguage === 'english' ? '6. Medal Concluding Prayer' : '6. Sala ya Kuhitimisha', prayerName: 'Prayer to St. Michael',
  ...
  const buildSevenSorrowsBeads = (): Bead[] => {
    ...
    list.push({
      id: 1, type: 'intro', label: rosaryLanguage === 'english' ? '1. Opening Act of Contrition' : '1. Sala ya Toba ya Kuanzia', prayerName: 'Act of Contrition',
    ...
    for (let s = 0; s < 7; s++) {
      ...
      list.push({
        id: beadId++, type: 'sorrow', label: rosaryLanguage === 'english' ? `2. Sorrow ${s+1} - Meditation` : `2. Masikitiko ya ${s+1} - Tafakari`, prayerName: `The ${s+1} Sorrow`,
      ...
      for (let h = 0; h < 7; h++) {
        ...
        list.push({
          id: beadId++, type: 'hail-mary', label: rosaryLanguage === 'english' ? `3. Sorrow ${s+1} - Hail Mary ${h+1}` : `3. Masikitiko ya ${s+1} - Salamu Maria ${h+1}`, prayerName: prayersTemplates.hailMary.nameEn,
          prayerTextEn: prayersTemplates.hailMary.en, prayerTextSw: prayersTemplates.hailMary.sw,
    ...
    list.push({
      id: beadId++, type: 'conclusion', label: rosaryLanguage === 'english' ? '4. Concluding Prayer' : '4. Sala ya Kuhitimisha', prayerName: 'Concluding Intercession',
    ...
    <div className="space-y-8 md:space-y-12 pb-16 max-w-6xl mx-auto px-1">
      ...
      <div className="text-center space-y-2">
        <span className="inline-block text-xs font-bold text-primary uppercase tracking-widest bg-primary/10 px-3 py-1 rounded-full">
          {rosaryLanguage === 'english' ? 'Devotion & Spiritual Life' : 'Ibada na Maisha ya Kiroho'}
        </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-3 space-y-4">
            <div className="bg-card border border-border p-4 rounded-2xl shadow-sm space-y-4">
              <h3 className="font-extrabold text-sm text-foreground">{t.selectDevotional}</h3>
              <div className="space-y-2">
                ...
                >
                  <span className="font-extrabold text-foreground text-xs">{t.marianTitle}</span>
                  <span>{rosaryLanguage === 'english' ? '5 Decades. Meditations on the life of Christ.' : 'Miongo 5. Tafakari za maisha ya Yesu Kristo.'}</span>
                </button>
                <button
                ...
                >
                  <span className="font-extrabold text-foreground text-xs">{t.stmichaelTitle}</span>
                  <span>{rosaryLanguage === 'english' ? '9 Salutations in honor of the 9 Angel Choirs.' : 'Salamu 9 kwa heshima ya Kwaya 9 za Malaika.'}</span>
                </button>
                <button
              ...
              {selectedRosary === 'marian' && (
                <div className="space-y-2 pt-3 border-t border-border">
                  <h4 className="font-extrabold text-xs text-foreground uppercase tracking-wider">{t.mysteries}</h4>
                  <div className="grid grid-cols-2 gap-1.5">
                    {(['joyful', 'luminous', 'sorrowful', 'glorious'] as const).map((g) => (
                      ...
                      >
                        {rosaryLanguage === 'english' ? g : (g === 'joyful' ? 'Furaha' : g === 'luminous' ? 'Mwanga' : g === 'sorrowful' ? 'Uchungu' : 'Utukufu')}
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <div className="lg:col-span-4 bg-card border border-border p-4 rounded-2xl shadow-sm flex flex-col items-center">
            ...
            <div className="relative w-full aspect-[400/480] max-w-[340px] mt-4 flex items-center justify-center bg-muted/10 rounded-xl border border-border/40 p-2">
              <svg viewBox="0 0 400 480" className="w-full h-full select-none">
                <ellipse cx="200" cy="150" rx="115" ry="80" fill="none" stroke="currentColor" strokeWidth="2" className="text-border/60" />
                <line x1="200" y1="230" x2="200" y2="440" stroke="currentColor" strokeWidth="2.5" className="text-border/60" />
                ...
                {beads.map((b, idx) => {
                  ...
                  if (isCrucifix) {
                    return (
                      <g key={idx} className="cursor-pointer transition-all duration-300" onClick={() => setCurrentBeadIndex(idx)}>
                        <rect x={b.x - 14} y={b.y - 4} width="28" height="8" rx="2" fill={fill} stroke={stroke} strokeWidth={strokeWidth} />
                        <rect x={b.x - 4} y={b.y - 20} width="8" height="36" rx="2" fill={fill} stroke={stroke} strokeWidth={strokeWidth} />
                        {isActive && (
                          <circle cx={b.x} cy={b.y} r="22" fill="none" stroke="#22c55e" strokeWidth="2" strokeDasharray="4,4" className="animate-spin" style={{ animationDuration: '6s' }} />
                        )}
                      </g>
                    );
                  }
                  ...
                  return (
                    <g key={idx} className="cursor-pointer transition-all duration-300" onClick={() => setCurrentBeadIndex(idx)}>
                      {isActive && <circle cx={b.x} cy={b.y} r={radius + 6} fill="rgba(34, 197, 94, 0.15)" className="animate-pulse" />}
                      <circle cx={b.x} cy={b.y} r={radius} fill={fill} stroke={stroke} strokeWidth={strokeWidth} className="transition-all hover:scale-125" />
                    </g>
                  );
                })}
              </svg>
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden flex flex-col">
              ...
              <div className="bg-muted/40 p-4 border-b border-border/80 flex justify-between items-center">
                <div>
                  <span className="inline-block text-[9px] font-extrabold uppercase tracking-widest bg-primary/10 text-primary border border-primary/20 px-2 py-0.5 rounded-full">
                    {t.step} {currentBeadIndex + 1} {t.of} {beads.length}
                  </span>
              ...
              {(currentBead.mysteryTextEn || currentBead.mysteryTextSw) && (
                <div className="p-4 bg-primary/5 border-b border-border/60 text-xs text-foreground/90 space-y-1.5">
                  <h5 className="font-black text-[9px] uppercase text-primary tracking-widest">{t.meditation}</h5>
              ...
              <div className="p-4 border-t border-border flex justify-between items-center bg-muted/20">
                ...
                >
                  <span>{currentBeadIndex === beads.length - 1 ? t.startOver : t.next}</span>
            ...
            <div className="bg-card border border-border p-4 rounded-2xl shadow-sm flex items-center justify-between">
              <span className="text-xs font-black text-foreground">{t.decadeGuide}</span>
      ...
      {activeTab === 'pdfs' && (
        <div className="space-y-4">
          {loadingPdfs ? (
            ...
            <div className="text-center py-10 text-xs font-semibold text-muted-foreground bg-card border border-border rounded-2xl">
              {t.noPdfs}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {pdfList.map((pdf, idx) => (
                <div key={pdf.id || idx} className="bg-card border border-border p-5 rounded-2xl shadow-sm flex flex-col justify-between space-y-4 hover:shadow-md transition-all">
                  <div className="space-y-2">
                    <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary">
                      <FileText className="w-5.5 h-5.5" />
                    </div>
                    <h3 className="font-extrabold text-sm text-foreground">
                      {rosaryLanguage === 'english' ? pdf.title : (pdf.title.includes('(English)') ? pdf.title.replace('(English)', '(Kiingereza)') : pdf.title)}
                    </h3>
                    <p className="text-xs text-muted-foreground leading-relaxed">{pdf.description}</p>
                  </div>
                  <div className="flex items-center justify-between pt-2 border-t border-border/60">
                    <span className="text-[10px] font-bold text-muted-foreground">{t.size} {pdf.size || 'N/A'}</span>
                    <a
                      href={pdf.file_url}
                      target="_blank"
